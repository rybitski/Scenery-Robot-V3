...
void loop() {
  ...
  if(!eStopFlag){ //if we are not in an emergency condition
    if(updateCMDCode()){  //If a new CMD code was parsed (note that updateCMDCode also strips the length bit)
...
void cueControlMode(){
  ...
  if(a==-1){
    ...
    digitalWrite(brake, HIGH);  //remove power to the brakes to engage them.
    ST1.motor(1,a);
    ST1.motor(2,b);
    readEncoders();
    //p.write(encoder1Reading);
    Bridge.put("A0",encoder1Reading);
    //p.write(encoder12Reading);
    Bridge.put("A1",encoder2Reading);
  }
}
